...
import { ControlledMonacoDiffEditor, ControlledMonacoEditor, } from "../../components/monaco/MonacoEditor";
						...
						<div className="row">
							<div className="span12">
								...
								<p>
									...
									<a target="_blank" href={`https://registry.npmjs.org/monaco-editor/-/monaco-editor-${monacoEditorVersion}.tgz`}>
										download link
									</a>{" "}
									or{" "}
				...
				<footer className="container">
					...
					<p className="text-center">
						<a href="https://microsoft.com" title="Microsoft">
							<img src="https://opensource.microsoft.com/assets/images/Microsoft_logo.svg" alt="Microsoft" style={{
                maxHeight: "23px",
                marginBottom: "12px",
            }}/>
						</a>
...
let EditorDemo = class EditorDemo extends React.Component {
    languages = new ObservablePromise(loadMonaco().then((m) => {
        const languages = m.languages.getLanguages();
        this.currentLanguage = languages.find((l) => l.id === "typescript");
        return languages;
    }));
    currentLanguage = undefined;
    currentTheme = themes[0];
    get currentSample() {
        return new ObservablePromise(this.currentLanguage
            ? this.loadSample(this.currentLanguage.id)
            : Promise.resolve(undefined));
    }
    async loadSample(name) {
        const result = (await import(`../../data/home-samples/sample.${name}.txt`)).default;
        return result;
    }
    render() {
        return (<div className="p-5 mb-4">
				...
				<div className="mt-4 row row-cols-2">
					<div className="col">
						...
						<Select values={this.languages.value || []} getLabel={(l) => l.id} value={ref(this, "currentLanguage")}/>
					</div>
					<div className="col">
						<label className="control-label">Theme</label>
						<Select values={themes} getLabel={(l) => l.name} value={ref(this, "currentTheme")}/>
					</div>
				</div>

				<div className="mt-2 editor-container" style={{ height: 500 }}>
					<ControlledMonacoEditor value={this.currentSample.value || "loading..."} language={this.currentLanguage?.id} theme={this.currentTheme.id} onDidValueChange={() => { }}/>
				</div>
			</div>);
    }
};
__decorate([
    observable.ref
], EditorDemo.prototype, "currentLanguage", void 0);
__decorate([
    observable.ref
], EditorDemo.prototype, "currentTheme", void 0);
__decorate([
    computed
], EditorDemo.prototype, "currentSample", null);
...
class DiffEditorDemo extends React.Component {
    render() {
        return (<div className="p-5 mb-4">
				...
				box.
				<div className="mt-2 editor-container" style={{ height: 500 }}>
					<ControlledMonacoDiffEditor originalValue={require("../../data/diff-sample/original.txt")
                .default} modifiedValue={require("../../data/diff-sample/modified.txt")
