 ...
 */
export function runTsc(_projectPath) {
    const projectPath = path.join(REPO_ROOT, _projectPath);
    console.log(`Launching compiler at ${_projectPath}...`);
    const res = cp.spawnSync(process.execPath, [path.join(__dirname, '../node_modules/typescript/lib/tsc.js'), '-p', projectPath], { stdio: 'inherit' });
 ...
 */
export function prettier(_filePath) {
    const filePath = path.join(REPO_ROOT, _filePath);
    cp.spawnSync(process.execPath, [path.join(__dirname, '../node_modules/prettier/bin-prettier.js'), '--write', filePath], { stdio: 'inherit' });
    console.log(`Ran prettier over ${_filePath}`);
}
/**
 * Transform an external .d.ts file to an internal .d.ts file
 */
export function massageAndCopyDts(source, destination, namespace) {
...
export function readFiles(pattern, options) {
...
export function writeFiles(files, dest) {
