...
async function prepareMonacoEditorCoreReleaseStableOrNightly() {
    const monacoEditorPackageJson = require(monacoEditorPackageJsonPath);
    let version;
    let ref;
...
async function prepareMonacoEditorCoreRelease(version, vscodeRef) {
    ...
    await group('Checkout vscode', async () => {
        const result = await gitShallowClone(vscodePath, 'https://github.com/microsoft/vscode.git', vscodeRef);
    ...
    await group('Checkout vscode-loc', async () => {
        ...
        'dependencies/vscode-loc', 'https://github.com/microsoft/vscode-loc.git', 'main');
    });
    await group('Set Version', async () => {
        const monacoEditorCorePackageJsonSourcePath = resolve(vscodePath, './build/monaco/package.json');
        const packageJson = require(monacoEditorCorePackageJsonSourcePath);
...
//# sourceMappingURL=monaco-editor-core-prepare.js.map