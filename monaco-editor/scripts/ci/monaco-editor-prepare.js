...
import { getNightlyVersion, group, run, writeJsonFile, gitCommitId } from '../lib';
import { getNightlyEnv } from './env';
const selfPath = __dirname;
const rootPath = join(selfPath, '..', '..');
const monacoEditorPackageJsonPath = resolve(rootPath, 'package.json');
const monacoEditorCorePackageJsonPath = resolve(rootPath, 'node_modules', 'monaco-editor-core', 'package.json');
async function prepareMonacoEditorReleaseStableOrNightly() {
    const monacoEditorPackageJson = JSON.parse(await readFile(monacoEditorPackageJsonPath, { encoding: 'utf-8' }));
    let version;
    ...
    if (arg === 'stable') {
        ...
        version = getNightlyVersion(monacoEditorPackageJson.version, getNightlyEnv().PRERELEASE_VERSION);
    }
    else {
...
async function prepareMonacoEditorRelease(monacoEditorCoreVersion) {
    ...
    await group('Set Version & Update monaco-editor-core Version', async () => {
        const packageJson = JSON.parse(await readFile(monacoEditorPackageJsonPath, { encoding: 'utf-8' }));
    ...
    await group('Setting vscode commitId from monaco-editor-core', async () => {
        const monacoEditorCorePackageJson = JSON.parse(await readFile(monacoEditorCorePackageJsonPath, { encoding: 'utf-8' }));
        const packageJson = JSON.parse(await readFile(monacoEditorPackageJsonPath, { encoding: 'utf-8' }));
...
//# sourceMappingURL=monaco-editor-prepare.js.map