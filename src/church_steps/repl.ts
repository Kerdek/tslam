import { evaluate_one, Graph, make, read, to_digraph } from "./church.js"
...
cmd.onkeydown = e => {
  ...
  const preceding_boundary: (c: number) => number = _c => 0
  const successive_boundary: (c: number) => number = _c => 0
...
const dispatch = async () => {
  ...
  try {
    ...
    const cmd_echo = system.cloneNode(true) as HTMLDivElement
    ...
    p.innerText = (err as Error).toString();
...
const reset = () => {
output.innerHTML = ''
state = undefined as unknown as Graph
dispatch_cb = e => e
step = 0
set_prompt('#') }
