...
export type Normal = Abs | Ref | Lit | Blt
export type Tree = Normal | App | Ref | Elm | Ind
...
const
  ...
  include: AsyncProcess = async () => {
    ...
    if (!res.ok) {
      fatal(`HTTP status ${res.status} while requesting \`./${r}\``) }
    x = `${await res.text()})${x}`
...
const s: GraphProcess = visit({
  ...
  ref: e => ret([e, [e[1]]]),
  ext: e => call(s(e[3]), ([dy, dyu]) => ret([make("ext", e[1], e[2], dy), dyu])),
...
const s: ExtProcess = e => () => call(visit({
  ...
  ref: y => y[1] === e[1] ? ret(make("ind", e[2])) : ret(y),
