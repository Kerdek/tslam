...
const par = (...elems) => create_element('p', function () { }, elems);
const header = create_element('h1', function () { }, [
    t(`Lambda Calculus`)
]);
const intro = par(t(`Lambda calculus is a programming language used by Alonzo Church to describe his theory of computation. Its only features are the abilities to define and use functions. It serves as a good proving ground for the design of interpreters, compilers, and other programming languages.`));
const grammar_remark = par(t(`The following is an unambiguous BNF grammar for the syntax of lambda calculus.`));
const indented_paragraph = (...elems) => create_element('p', function () {
    this.style.textIndent = "-30pt";
    this.style.paddingLeft = "30pt";
}, elems);
const grammar_ref = text => create_element('span', function () { }, [
...
const grammar_str = text => create_element('span', function () { }, [
    create_element('span', function () { this.style.color = "var(--lambda)"; }, [t('"')]),
    create_element('span', function () { this.style.color = "var(--string)"; }, [t(text)]),
    create_element('span', function () { this.style.color = "var(--lambda)"; }, [t('"')])
]);
const grammar_equal = () => create_element('span', function () { }, [
    create_element('span', function () { this.style.color = "var(--lambda)"; }, [t('::=')])
]);
const grammar_dots = () => create_element('span', function () { }, [
    create_element('span', function () { this.style.color = "var(--lambda)"; }, [t('...')])
]);
const grammar_or = () => create_element('span', function () { }, [
...
const ambiguity_remark = par(t(`Try to collect these two points from that:`));
...
const eval_rules = par(t(`The rules are simple. When the term in question is an application of an abstraction, it is reduced by manner of substitution. (press \`Evaluate\` in the following box). Such a term is called a redex, meaning it's a candidate for a reduction step.`));
const pg0 = playground('(λa b.b) "foo" "bar"');
const substitution_remark = par(t("This is just like when you were asked to substitute variables into equations in grade school. Ben Lynn uses the follwing example. With the help of some built-in arithmetic, we can see that `(λx y.sqrt (add (mul x x) (mul y y))) 3 4` means to substitute `x = 3` and `y = 4` in `sqrt (add (mul x x) (mul y y))`."));
...
document.body.append(header, intro, grammar_remark, grammar_box, ambiguity_remark, rules_box, eval_rules, pg0, substitution_remark, pg1, problems_remark, case_remark, steps_box, alternative_remark, steps_box2, interpreter_remark, let_remark, pg2, io_remark, pg3, sophisticated_remark, pg4, self_parse_remark, pg5);
