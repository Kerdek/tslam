...
import { highlight_html, read, to_digraph, to_outline } from "./lang.js";
import { make, reduce, unthunk } from "./graph.js";
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield include("./viz.js");
    yield include("./lite.render.js");
  ...
  fill: var(--background); }`);
    ...
    const text_output = make_checkbox('text_output', 'Text Output'), expand_thunks = make_checkbox('expand_thunks', 'Expand Thunks'), graph_output = make_checkbox('graph_output', 'Show Graphs'), keep_steps = make_checkbox('keep_steps', 'Keep Steps');
    ...
    cmd.onkeydown = e => {
        ...
        if (e.ctrlKey) {
            if (e.key === 'a') {
                ...
                r.setEnd(cmd.childNodes[0], ((_a = cmd.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0);
                z.addRange(r);
                return false;
            }
        ...
        const preceding_boundary = c => {
            ...
            if (c === 0)
                return 0;
            c--;
            for (; c !== raa && (!/\s/.test(text[c - 1]) || /\s/.test(text[c])); c--)
                ;
            return c;
        };
        const successive_boundary = c => {
            let [_ra, rb] = ranges(c);
            if (!rb)
                return text.length;
            let [_rba, rbb, rbk] = rb;
            ...
            if (c === text.length)
                return text.length;
            c++;
            for (; c !== rbb && (/\s/.test(text[c - 1]) || !/\s/.test(text[c])); c++)
                ;
            return c;
        };
        ...
        const [textp, anchorp, focusp] = e.key === '\\' ? (() => {
    ...
    const dispatch = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const cmd_echo = system_shadow.cloneNode(true);
        ...
        for (;;) {
            if (!keep_steps.checked)
                output_segment.innerHTML = '';
            ...
            const expand = (expand_thunks.checked ? unthunk : (x) => x)(state);
            ...
            if (graph_output.checked) {
                output_element.appendChild(yield to_digraph("state", { '': expand }));
            }
...
//# sourceMappingURL=webrepl.js.map